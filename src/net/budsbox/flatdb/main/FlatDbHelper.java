...
public abstract class FlatDbHelper {
	...
	public static void printToConsole(String directory, String recordType,
			ArrayList<String> attributes, PmManager stats) {
	...
	public static void printToFile(String inDir, String outDir,
			String recordType, ArrayList<String> attributes, PmManager stats) {
	...
	public static ArrayList<String> parseRow(DbnRow rp, String fieldName) {
		...
		String fieldType = rp.getRow().split(fieldName)[1].split(":")[1]
				.split("[0-9]")[0];
	...
	private static ArrayList<String> parseRowString(DbnRow rp, String fieldName) {
		...
		if (rp.getAttributes().size() < indexPlusOne) {
			fieldIndex2 = rp.getAttributes().indexOf(
					rp.getAttributes().get(indexPlusOne));
		} else {
			fieldIndex2 = rp.getRow().indexOf("]]]");
		}

		str1 = rp.getRow().substring(
				rp.getAttributeIndexArray().get(fieldIndex1), fieldIndex2);
		...
		cleanValue = str1
				.substring(fieldLength + stringSizeLength,
	...
	private static ArrayList<String> parseRowArray(DbnRow rp, String fieldName) {
		int arraySize = Integer.parseInt(rp.getRow().split(fieldName)[1]
				.toString().split("\\[")[0].split("A")[1]);
		...
		for (int i = 1; i <= arraySize; i++) {
			if (i < arraySize) {
				...
				arrayValues.add(cutRow.substring(cutRow.indexOf(pattern1),
						cutRow.indexOf(pattern2)).split("\"")[1]);
			} else if (i == arraySize) {
				String pattern1 = "ix" + fieldArrayindex;
				arrayValues.add(cutRow.substring(cutRow.indexOf(pattern1),
						cutRow.indexOf("]")).split("\"")[1]);
			}
			fieldArrayindex++;
		}
		return arrayValues;
